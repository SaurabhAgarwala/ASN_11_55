...
def weights_download(out='model/yolov3.weights'):
    '''
    Call this function, if you haven't download the yolov3.weights
    '''
    _ = wget.download('https://pjreddie.com/media/files/yolov3.weights', out='model/yolov3.weights')
...
def predict(image, prep_img):
    ...
    for i in range(nums[0]):
        ...
        if int(classes[0][i] == 67):
            not_allowed_objects.append('mobile')
            print('Mobile Phone detected')
        if int(classes[0][i] == 62):
            not_allowed_objects.append('tvmonitor')
            print('Monitor detected')
        if int(classes[0][i] == 63):
            not_allowed_objects.append('laptop')
...
#     print('Download the YoloV3 Model Weights')
    while(True):
        ...
        for file in files:
            ...
            # Show result
            # cv.imshow('Predict Image', result)
            # cv.waitKey(0)

            # return result
            if len(result)>0:
                print('Not allowed objects detected')
            url = 'http://localhost:8000/proctor/reportviolation/'+candidate_id+'/othergadgets/'
            print(url)
            requests.get(url)
